...
export const useStationsStore = defineStore('stations', () => {
  ...
  async function fetchStations(filters: StationFilters = {}) {
    ...
    try {
      ...
      const message = err.response?.data?.message || 'Failed to fetch stations';
      error.value = message;
      toast.error(message);
      return [];
    } finally {
  ...
  async function fetchStationById(id: string) {
    ...
    try {
      ...
      const message = err.response?.data?.message || 'Failed to fetch station details';
      error.value = message;
      toast.error(message);
      return null;
    } finally {
  ...
  async function createStation(stationData: Omit<ChargingStation, '_id' | 'owner' | 'createdAt' | 'updatedAt'>) {
    ...
    try {
      ...
      const message = err.response?.data?.message || 'Failed to create station';
      error.value = message;
      toast.error(message);
      return null;
    } finally {
  ...
  async function updateStation(id: string, stationData: Partial<ChargingStation>) {
    ...
    try {
      ...
      const message = err.response?.data?.message || 'Failed to update station';
      error.value = message;
      toast.error(message);
      return null;
    } finally {
  ...
  async function deleteStation(id: string) {
    ...
    try {
      ...
      const message = err.response?.data?.message || 'Failed to delete station';
      error.value = message;
      toast.error(message);
      return false;
    } finally {
